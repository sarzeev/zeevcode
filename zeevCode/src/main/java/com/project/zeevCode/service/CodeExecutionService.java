...
public class CodeExecutionService {
        ...
        try {
            for (TestCase testCase : testCases) {
                ...
                if (result != ExecutionResult.ACCEPTED) {
                    ...
                    sendWebSocketUpdate(matchId.toString(), userId.toString(), status.name(), "Test case failed.");
                    return;
                }
            }

            submissionService.updateSubmissionStatus(submissionId, SubmissionStatus.ACCEPTED, 0);
            sendWebSocketUpdate(matchId.toString(), userId.toString(), "ACCEPTED", "All test cases passed!");

            matchService.finishMatch(matchId, userId);
            sendWebSocketUpdate(matchId.toString(), userId.toString(), "MATCH_FINISHED", "Match has finished.");
            ...
            sendWebSocketUpdate(matchId.toString(), userId.toString(), "RUNTIME_ERROR", "An unexpected error occurred.");
        }
    }
    ...
    private void sendWebSocketUpdate(String matchId, String userId, String status, String message) {
