...
@Service
@ConditionalOnProperty(name = "execution.provider", havingValue = "native")
@RequiredArgsConstructor
@Slf4j
public class NativeExecutionProvider implements ExecutionProvider {

    private static final int MAX_OUTPUT_SIZE = 256 * 1024; // 256 KB
    ...
    @org.springframework.beans.factory.annotation.Value("${execution.isolated.enabled:false}")
    private boolean isolatedEnabled;

    @org.springframework.beans.factory.annotation.Value("${execution.isolated.user:code_runner}")
    private String isolatedUser;

    @org.springframework.beans.factory.annotation.Value("${execution.isolated.script:./run_isolated.sh}")
    private String isolatedScript;

    private String[] wrapCommand(String[] command) {
        if (!isolatedEnabled || System.getProperty("os.name").toLowerCase().contains("win")) {
            return command;
        }
        java.util.List<String> finalCommand = new java.util.ArrayList<>();
        finalCommand.add("sudo");
        finalCommand.add("-u");
        finalCommand.add(isolatedUser);
        finalCommand.add("bash");
        finalCommand.add(isolatedScript);
        finalCommand.addAll(java.util.Arrays.asList(command));
    ...
    public void executeSubmission(UUID submissionId, String code, Language language,
        ...
        try {
            ...
            log.error("Error during native code execution for submission {}", submissionId, e);
    ...
    public void executePracticeSubmission(UUID submissionId, String code, Language language,
        ...
        try {
            ...
            log.error("Error during native practice execution for submission {}", submissionId, e);
            submissionService.updateSubmissionStatus(submissionId, SubmissionStatus.RUNTIME_ERROR, null);
        ...
        try {
            ...
            switch (language) {
                ...
                case CPP:
                    ...
                    runCommand = new String[]{tempDir.resolve(execName).toAbsolutePath().toString()};
                    break;
            }
            ...
            if (!System.getProperty("os.name").toLowerCase().contains("win")) {
                tempDir.resolve(sourceFileName).toFile().setReadable(true, false);
            }

            if (compileCommand != null) {
                ProcessResult compileResult = executeProcess(wrapCommand(compileCommand), tempDir, null);
            ...
            ProcessResult processResult = executeProcess(wrapCommand(runCommand), tempDir, input);
            ...
            // Treat trailing spaces neutrally to avoid pedantic failures
    ...
    private ProcessResult executeProcess(String[] command, Path directory, String input)
        ...
        if (input != null) {
            try (BufferedWriter writer = new BufferedWriter(
                ...
                // Ignore broken pipe if process closes stdin immediately
            }
        }
        ...
        try (BufferedReader reader = new BufferedReader(
            ...
            while ((read = reader.read(buffer)) != -1) {
                if (totalBytesRead + read > MAX_OUTPUT_SIZE) {
                    output.append(buffer, 0, MAX_OUTPUT_SIZE - totalBytesRead);
